...
class DataAnalyzerTool:
    ...
    def _validate_data(self, data):
        ...
        if data["mrr"] > data["revenue"]: # MRR is usually a subset of total revenue for a period
    ...
    def analyze(self, data):
        ...
        phase_threshold_next = self.BUSINESS_PHASES[current_phase]["max_mrr"]
        
        issues = []
        recommendations = []
        phase_progression = "stay"

        logging.info(f"{{\"message\": \"Starting analysis for MRR: {mrr}, Phase: {current_phase}\", \"data\": {data}}}")

        # Analyze against benchmarks
        # Conversion Rate
        expected_cr = self.BENCHMARKS["conversion_rate"].get(current_phase, 0.01)
        if data["conversion_rate"] < expected_cr:
            issues.append(f"Conversion rate ({data['conversion_rate']:.2%}) is below benchmark ({expected_cr:.2%}) for {current_phase} phase.")
            recommendations.append("Prioritize A/B testing landing pages and optimizing conversion funnels.")

        # Churn Rate
        expected_churn = self.BENCHMARKS["churn_rate"].get(current_phase, 0.10)
        if data["churn_rate"] > expected_churn:
            issues.append(f"Churn rate ({data['churn_rate']:.2%}) is above benchmark ({expected_churn:.2%}) for {current_phase} phase.")
            recommendations.append("Focus on customer success and retention strategies. Identify reasons for churn.")

        # CAC/LTV Ratio
        expected_cac_ltv_ratio = self.BENCHMARKS["cac_ltv_ratio"].get(current_phase, 1.0)
        if data["cac"] > 0 and (data["ltv"] / data["cac"]) < expected_cac_ltv_ratio:
            issues.append(f"LTV/CAC ratio ({data['ltv'] / data['cac']:.2f}) is below benchmark ({expected_cac_ltv_ratio:.2f}) for {current_phase} phase.")
        ...
        elif data["cac"] == 0 and mrr > 0: # If there's MRR but no CAC, it could be misleading (e.g., all organic initial growth)
        ...
        if mrr < self.BUSINESS_PHASES[current_phase]["min_mrr"]:
            # This case ideally shouldn't happen if get_current_phase is correct, but as a safeguard
        ...
        if mrr >= phase_threshold_next and current_phase != "mature_50k_plus":
        ...
        elif mrr < phase_threshold_next * 0.5 and current_phase != "discovery_0_100":
        ...
        if not issues:
            ...
            if phase_progression == "stay": # Add a positive recommendation if no issues and not advancing
        ...
        analysis_result = {
            ...
            "issues": sorted(list(set(issues))), # Remove duplicates and sort
            "recommendations": sorted(list(set(recommendations)), reverse=True), # Prioritize advancing message if present
